...
- **Tier assignment:** demoting a mutant to a lower tier (e.g. B0 → B2) moves it from zero-tolerance to the 5% band without touching any denominator. Nothing in the CSV proves a tier label — tier truth is a reviewer check against `requirements.csv` risk levels. Enforce it: `python3 verdictgate.py results.csv --requirements requirements.csv` refuses tier mismatches and unknown behaviors (exit 2); without the flag the verdict stamps `requirements-cross-check: off (tiers unverified)`.

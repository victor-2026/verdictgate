...
- **PRECONDITION (locked 2026-09-24):** `verdictgate.py` is at 32750 B, 18 B under the 32 KiB cap. The NEXT touch of this file MUST move `run_rmt()` + `get_line_number()` into `rmt.py` (imported, not duplicated) — this also removes the double `from rmt import`. No feature edits to `verdictgate.py` until the split lands, pilot pressure or not.
